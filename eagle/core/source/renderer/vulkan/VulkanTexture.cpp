...
VulkanTexture::VulkanTexture(const TextureCreateInfo &textureCreateInfo,
    ...
    m_image(std::make_shared<VulkanImage>(
            m_usage == TextureUsage::STORAGE ? DescriptorType::STORAGE_IMAGE : DescriptorType::SAMPLED_IMAGE,
...
void
VulkanTexture::copy_buffer_to_image(const Reference<VulkanBuffer>& buffer, VkImage image, uint32_t width,
                                      uint32_t height) {
...
Handle<Image> VulkanTexture::get_image() {
    return m_image;
}

Reference<VulkanBuffer> VulkanTexture::create_staging_buffer() {
